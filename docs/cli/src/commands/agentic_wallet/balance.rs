...
/// Cache TTL for the active (current) account balance query.
const ACTIVE_BALANCE_TTL: i64 = 15;
...
pub(super) async fn cmd_balance(
    ...
    let mut wallets =
        wallet_store::load_wallets()?.ok_or_else(|| anyhow::anyhow!("not logged in"))?;
    if cfg!(feature = "debug-log") {
        eprintln!(
    ...
    if all {
        ...
        if account_ids.is_empty() {
            bail!("no wallet accounts found in wallets.json");
        }
    ...
    if cfg!(feature = "debug-log") {
        eprintln!(
            "[DEBUG][cmd_balance] resolved account_id={}",
            account_id
        );
    }
    ...
    if let Some(token_addr_str) = token_address {
        ...
        let chain_entry = super::chain::get_chain_by_name(c)
            .await?
        ...
        let chain_index = chain_entry["chainIndex"]
            .as_i64()
            .map(|n| n.to_string())
            .or_else(|| chain_entry["chainIndex"].as_str().map(|s| s.to_string()))
    ...
    // ── Scenario 2: No flags — show all accounts with EVM/SOL addresses + balance ──
    if cfg!(feature = "debug-log") {
    ...
    let all_account_ids: Vec<String> = if !wallets.accounts.is_empty() {
        wallets
            .accounts
            .iter()
        ...
        wallets.accounts_map.keys().cloned().collect()
    };
    if cfg!(feature = "debug-log") {
        eprintln!(
            "[DEBUG][cmd_balance] scenario=default, all_account_ids={:?}",
            all_account_ids
        );
    }
    ...
    let balance_cache = if !force {
        wallet_store::get_batch_balance_cache(BATCH_BALANCE_TTL)?
    } else {
        None
    };
    let cache_hit = balance_cache.is_some();
    if cfg!(feature = "debug-log") {
        eprintln!(
            "[DEBUG][cmd_balance] scenario=default, cache_hit={}",
            cache_hit
        );
    }
    let balance_cache = if let Some(cached) = balance_cache {
        cached
    } else {
        let ids_joined = all_account_ids.join(",");
        let data = client
            .balance_batch(&access_token, &ids_joined)
            .await
            .map_err(format_api_error)?;
        if cfg!(feature = "debug-log") {
            eprintln!(
                "[DEBUG][cmd_balance] scenario=default, balance_batch response_len={}",
                data.to_string().len()
            );
        }
        ...
        wallet_store::set_batch_balance_cache(&entries)?;
        wallet_store::load_balance_cache()?
    };

    let total_usd = sum_cache_total(&balance_cache);
    if cfg!(feature = "debug-log") {
        eprintln!(
            "[DEBUG][cmd_balance] scenario=default, total_usd={}, cached_accounts_count={}",
            total_usd, balance_cache.accounts.len()
        );
    }
    let active_account_id = wallets.selected_account_id.clone();

    let accounts_out: Vec<Value> = all_account_ids
        .iter()
        .map(|aid| {
            let account_name = wallets
                .accounts
                .iter()
                .find(|a| &a.account_id == aid)
                .map(|a| a.account_name.as_str())
                .unwrap_or("");

            let evm_address = get_evm_address(&wallets, aid);
            let sol_address = get_sol_address(&wallets, aid);

            let account_total = balance_cache
                .accounts
                .get(aid)
                .map(|e| e.total_value_usd.as_str())
                .unwrap_or("0.00");

            let is_active = aid == &active_account_id;
