...
pub async fn get_real_chain_index(chain_index: &str) -> Result<u64> {
    let entry = get_chain_by_index(chain_index)
        .await?
        .ok_or_else(|| anyhow::anyhow!("Chain index {} not found in supported chains", chain_index))?;
    entry["realChainIndex"]
        .as_str()
        .and_then(|s| s.parse::<u64>().ok())
        .or_else(|| entry["realChainIndex"].as_u64())
        .ok_or_else(|| anyhow::anyhow!("Cannot resolve realChainIndex for chain index {}", chain_index))
}
