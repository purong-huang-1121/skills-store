    ...
    let resolved_account_id = match account_id {
        ...
        _ => {
            ...
            if wallets.selected_account_id.is_empty() {
                bail!("not logged in: no selectedAccountId");
            }
            wallets.selected_account_id
        }
    };

    // Resolve chain name → chainIndex
    let chain_index = match chain {
        Some(name) if !name.is_empty() => {
            let entry = super::chain::get_chain_by_name(name)
                .await?
                .ok_or_else(|| anyhow::anyhow!("unsupported chain: {name}"))?;
            entry["chainIndex"].as_i64().map(|n| n.to_string()).unwrap_or_default()
        }
        _ => String::new(),
    };
