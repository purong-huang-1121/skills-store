...
pub async fn execute(ctx: &Context, cmd: SecurityCommand) -> Result<()> {
    match cmd {
        ...
        SecurityCommand::Approvals {
            ...
            limit,
            cursor,
        } => approvals(ctx, &address, chain.as_deref(), risky, limit, cursor).await,
...
mod tests {
    ...
    fn default_base_url_is_beta() {
        assert_eq!(crate::client::DEFAULT_BASE_URL, "https://beta.okex.org");
    }
...
async fn approvals(
    ...
    let address_list: Vec<Value> = if let Some(chains_str) = chain {
        chains_str
            .split(',')
            .map(|c| c.trim())
            .filter(|c| !c.is_empty())
        ...
        // No chain filter — query all supported EVM chains (approvals are EVM-only)
