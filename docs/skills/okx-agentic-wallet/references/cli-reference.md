...
| `--chain` | No | all chains | Chain name (e.g., `ethereum`, `solana`, `xlayer`). Required when using `--token-address`. |
...
```bash
onchainos wallet balance --chain ethereum --token-address "0x3883ec817f2a080cb035b0a38337171586e507be"
# -> Display:
...
| `--chain-index` | string | No | Filter by chain index (e.g. "1" for Ethereum, "501" for Solana) |
...
| `--chain-index` | string | Yes | Chain index where the transaction occurred (e.g. "1" for Ethereum) |
...
> Either `--input-data` (EVM) or `--unsigned-tx` (Solana) must be provided. The CLI returns E101 if neither is present.
