...
# Usage: curl -sSL https://raw.githubusercontent.com/okx/plugin-store/main/reinstall.sh -o /tmp/reinstall.sh && sh /tmp/reinstall.sh
#
# 环境变量:
#   RAW_BASE_URL  脚本下载基础 URL（默认: https://raw.githubusercontent.com/okx/plugin-store/main）
#                 例如: RAW_BASE_URL=https://raw.githubusercontent.com/okx/plugin-store/main sh /tmp/reinstall.sh

set -e

RAW_BASE_URL="${RAW_BASE_URL:-https://raw.githubusercontent.com/okx/plugin-store/main}"
...
npx skills add okx/plugin-store --skill plugin-store --yes
SKILLS_EXIT=$?
...
curl -sSL "${RAW_BASE_URL}/install.sh" | RAW_BASE_URL="$RAW_BASE_URL" sh
...
if command -v onchainos >/dev/null 2>&1; then
  ...
  if [ $? -eq 0 ]; then
    ...
    echo "请按照文档升级: https://web3.okx.com/zh-hans/onchainos/dev-docs/home/install-your-agentic-wallet"
  fi
else
  ...
  echo "请按照以下文档安装:"
  echo ""
  echo "  https://web3.okx.com/zh-hans/onchainos/dev-docs/home/install-your-agentic-wallet"
  echo ""
fi
